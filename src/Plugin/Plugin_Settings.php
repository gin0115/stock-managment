...
class Plugin_Settings {
    ...
    protected function set_vars(): void
    {
        ...
        $this->use_location_bins = \property_exists($vars, self::LOCATION_BINS_KEY )
            ? $vars->self::LOCATION_BINS_KEY : true;
        $this->use_location_sites = \property_exists($vars, self::LOCATION_SITES_KEY)
            ? $vars->self::LOCATION_SITES_KEY : true;
        $this->use_pack_size_modifiers = \property_exists($vars, self::PACK_SIZE_MODIFIERS_KEY)
            ? $vars->self::PACK_SIZE_MODIFIERS_KEY : false;
        $this->use_pick_partial_orders = \property_exists($vars, self::PICK_PARTIAL_ORDER_KEY)
            ? $vars->self::PICK_PARTIAL_ORDER_KEY : true;
    }
    ...
    protected function save()
    {
        ...
        return $this->options_repository->set_as_autoloaded(self::SETTINGS_KEY, $vars);
    }
