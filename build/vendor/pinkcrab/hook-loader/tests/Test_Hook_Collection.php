...
class Test_Hook_Collection extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $collection = new \PC_Woo_Stock_Man\PinkCrab\Loader\Hook_Collection();
        $collection->push(new \PC_Woo_Stock_Man\PinkCrab\Loader\Hook('action', 'is_string'));
    ...
    {
        $data = new \PC_Woo_Stock_Man\PinkCrab\Loader\Hook('action', 'is_string');
        $collection = new \PC_Woo_Stock_Man\PinkCrab\Loader\Hook_Collection();
    ...
    {
        $inital_hook = new \PC_Woo_Stock_Man\PinkCrab\Loader\Hook('action', 'is_string');
        $collection = new \PC_Woo_Stock_Man\PinkCrab\Loader\Hook_Collection();
        $collection->push($inital_hook);
        add_filter(\PC_Woo_Stock_Man\PinkCrab\Loader\Hook_Collection::REGISTER_HOOKS, function ($hooks) {
            // Change the first hooks handle.
            $hooks[0]->handle('changed_via_filter');
            // Add a custom hook.
            $hooks[] = new \PC_Woo_Stock_Man\PinkCrab\Loader\Hook('added_via_filter', 'is_string');
            return $hooks;
        });
        ...
        unset($GLOBALS['wp_filter'][\PC_Woo_Stock_Man\PinkCrab\Loader\Hook_Collection::REGISTER_HOOKS]);
    }
}
