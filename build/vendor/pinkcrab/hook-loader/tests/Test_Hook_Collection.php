...
class Test_Hook_Collection extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $collection = new \pc_stock_man_v1\PinkCrab\Loader\Hook_Collection();
        $collection->push(new \pc_stock_man_v1\PinkCrab\Loader\Hook('action', 'is_string'));
    ...
    {
        $data = new \pc_stock_man_v1\PinkCrab\Loader\Hook('action', 'is_string');
        $collection = new \pc_stock_man_v1\PinkCrab\Loader\Hook_Collection();
    ...
    {
        $inital_hook = new \pc_stock_man_v1\PinkCrab\Loader\Hook('action', 'is_string');
        $collection = new \pc_stock_man_v1\PinkCrab\Loader\Hook_Collection();
        $collection->push($inital_hook);
        add_filter(\pc_stock_man_v1\PinkCrab\Loader\Hook_Collection::REGISTER_HOOKS, function ($hooks) {
            // Change the first hooks handle.
            $hooks[0]->handle('changed_via_filter');
            // Add a custom hook.
            $hooks[] = new \pc_stock_man_v1\PinkCrab\Loader\Hook('added_via_filter', 'is_string');
            return $hooks;
        });
        ...
        unset($GLOBALS['wp_filter'][\pc_stock_man_v1\PinkCrab\Loader\Hook_Collection::REGISTER_HOOKS]);
    }
}
