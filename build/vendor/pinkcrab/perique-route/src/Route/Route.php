...
class Route extends \pc_stock_man_v1\PinkCrab\Route\Route\Abstract_Route
{
    ...
    protected function translate_arguments(string $route) : string
    {
        ...
        $route = \preg_replace('/:(' . $allowed_chars . ')/', '(?<$1>' . $allowed_chars . ')', $route);
        // Create capture group for '{parameter}'
        $route = \preg_replace('/{(' . $allowed_chars . ')}/', '(?<$1>' . $allowed_chars . ')', \is_string($route) ? $route : '');
