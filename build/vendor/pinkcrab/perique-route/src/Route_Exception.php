...
class Route_Exception extends \Exception
{
    ...
    {
        return new self(\sprintf('Namespace not defined in %s', $route), 101);
    }
    /**
     ...
     * @code 102
     */
    public static function callback_not_defined(\pc_stock_man_v1\PinkCrab\Route\Route\Route $route) : self
    {
        // Set the namespace if exists.
        $namespace = '' !== $route->get_namespace() ? $route->get_namespace() : '_MISSING_NAMESPACE_';
        return new self(\sprintf('Callback not defined for [%s] %s%s', \strtoupper($route->get_method()), \strtoupper($namespace), \strtoupper($route->get_route())), 102);
    }
    /**
     ...
     * @return self
     */
    public static function invalid_http_method(\pc_stock_man_v1\PinkCrab\Route\Route\Route $route) : self
    {
