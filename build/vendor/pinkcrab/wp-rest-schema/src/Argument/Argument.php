...
class Argument
{
     ...
     * @return static
     */
    public static final function on(string $key, ?callable $config = null) : self
    {
        $class = \get_called_class();
        $argument = new $class($key);
        return $config ? $config($argument) : $argument;
    }
    /**
     ...
     * @return static
     */
    public function required(bool $required = \true) : self
    {
    ...
    {
        $this->expected = \is_array($this->expected) ? \array_merge($this->expected, $expected) : $expected;
        return $this;
    }
    /**
