...
class Array_Attribute_Parser extends \pc_stock_man_v1\PinkCrab\WP_Rest_Schema\Parser\Abstract_Parser
{
    /**
    ...
    public function parse_attributes() : array
    {
        ...
        if (!\is_a($this->argument, \pc_stock_man_v1\PinkCrab\WP_Rest_Schema\Argument\Array_Type::class)) {
            return array();
        }
        ...
        if (!empty($items)) {
            ...
            $attributes['items'] = 'allOf' === $relationship ? $items : array($relationship => $items);
        }
        // Min items
     ...
     */
    protected function parse_array_items(\pc_stock_man_v1\PinkCrab\WP_Rest_Schema\Argument\Array_Type $argument) : array
    {
        $items = array();
        ...
        if ($argument->item_count() === 1) {
            $items = \pc_stock_man_v1\PinkCrab\WP_Rest_Schema\Parser\Argument_Parser::as_list(\array_values($argument->get_items())[0]);
            // @phpstan-ignore-line, already checked if array empty.
        } else {
            foreach ($argument->get_items() ?? array() as $key => $value) {
                $items[] = \pc_stock_man_v1\PinkCrab\WP_Rest_Schema\Parser\Argument_Parser::as_list($value);
            }
        }
        return $items;
    }
