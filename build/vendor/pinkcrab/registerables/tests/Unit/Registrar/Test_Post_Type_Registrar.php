...
class Test_Post_Type_Registrar extends \PHPUnit\Framework\TestCase
{
    ...
    public function test_can_compile_args() : void
    {
        ...
        // Check labels
        $expected = array('name' => "{$cpt->plural}", 'singular_name' => "{$cpt->singular}", 'add_new' => 'Add New', 'add_new_item' => "Add New {$cpt->singular}", 'edit_item' => "Edit {$cpt->singular}", 'new_item' => "New {$cpt->singular}", 'view_item' => "View {$cpt->singular}", 'view_items' => "View {$cpt->plural}", 'search_items' => "Search {$cpt->singular}", 'not_found' => "No {$cpt->plural} found", 'not_found_in_trash' => "No {$cpt->plural} found in Trash", 'parent_item_colon' => "Parent {$cpt->singular}:", 'all_items' => "All {$cpt->plural}", 'archives' => "{$cpt->plural} Archives", 'attributes' => "{$cpt->plural} Attributes", 'insert_into_item' => "Insert into {$cpt->singular}", 'uploaded_to_this_item' => "Uploaded to this {$cpt->singular}", 'featured_image' => 'Featured image', 'set_featured_image' => 'Set featured image', 'remove_featured_image' => 'Remove featured image', 'use_featured_image' => 'Use as featured image', 'menu_name' => "{$cpt->plural}", 'filter_items_list' => "Filter {$cpt->plural} list", 'filter_by_date' => 'Filter by date', 'items_list' => "{$cpt->plural} list", 'item_published' => "{$cpt->singular} published", 'item_published_privately' => "{$cpt->singular} published privately", 'item_reverted_to_draft' => "{$cpt->singular} reverted to draft", 'item_scheduled' => "{$cpt->singular} scheduled", 'item_updated' => "{$cpt->singular} updated", 'item_link' => "{$cpt->singular} Link", 'item_link_description' => "A link to a {$cpt->singular}");
        foreach ($expected as $key => $value) {
