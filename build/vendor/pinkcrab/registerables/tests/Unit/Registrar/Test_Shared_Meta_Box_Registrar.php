...
class Test_Shared_Meta_Box_Registrar extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $registered = \false;
        $mb_registrar = $this->createMock(\PC_Woo_Stock_Man\PinkCrab\Registerables\Registrar\Meta_Box_Registrar::class);
        ...
        $registrar = new \PC_Woo_Stock_Man\PinkCrab\Registerables\Registrar\Shared_Meta_Box_Registrar($mb_registrar, $md_registrar);
        $registrar->register($this->createMock(\PC_Woo_Stock_Man\PinkCrab\Registerables\Registration_Middleware\Registerable::class));
    ...
    public function test_registers_shared_meta_boxes() : void
    {
        ...
        $md_registrar = $this->createMock(\PC_Woo_Stock_Man\PinkCrab\Registerables\Registrar\Meta_Data_Registrar::class);
        $registrar = new \PC_Woo_Stock_Man\PinkCrab\Registerables\Registrar\Shared_Meta_Box_Registrar($mb_registrar, $md_registrar);
        $registrar->register(new \PC_Woo_Stock_Man\PinkCrab\Registerables\Tests\Fixtures\Shared_Metabox\Post_Page_Meta_Box());
    ...
    {
        $data = array(new \PC_Woo_Stock_Man\PinkCrab\Registerables\Meta_Data('one'), new \stdClass(), new \PC_Woo_Stock_Man\PinkCrab\Registerables\Meta_Data('two'));
        $registrar = new \PC_Woo_Stock_Man\PinkCrab\Registerables\Registrar\Shared_Meta_Box_Registrar($this->createMock(\PC_Woo_Stock_Man\PinkCrab\Registerables\Registrar\Meta_Box_Registrar::class), $this->createMock(\PC_Woo_Stock_Man\PinkCrab\Registerables\Registrar\Meta_Data_Registrar::class));
        $filtered = \PC_Woo_Stock_Man\Gin0115\WPUnit_Helpers\Objects::invoke_method($registrar, 'filter_meta_data', array($data));
    ...
    public function test_throws_exception_for_invalid_meta_data() : void
    {
        ...
        $mock_meta_data = new \PC_Woo_Stock_Man\PinkCrab\Registerables\Meta_Data('rr');
        $mock_meta_data->type('array')->rest_schema(\true);
        // Mock the controller to return the mock meta data.
        $mock_controller = $this->createMock(\PC_Woo_Stock_Man\PinkCrab\Registerables\Tests\Fixtures\Shared_Metabox\Post_Page_Meta_Box::class);
        $mock_controller->method('meta_data')->willReturn(array($mock_meta_data));
        $mock_controller->method('meta_box')->willReturn(\PC_Woo_Stock_Man\PinkCrab\Registerables\Meta_Box::side('test')->screen('post'));
