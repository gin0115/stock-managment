 ...
 */
namespace pc_stock_man_v1\PinkCrab\Registerables\Tests\Unit\Registration_Middleware;
...
class Test_Registerable_Middleware extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $middleware = new \pc_stock_man_v1\PinkCrab\Registerables\Registration_Middleware\Registerable_Middleware();
        $loader = $this->createMock(\pc_stock_man_v1\PinkCrab\Loader\Hook_Loader::class);
        $middleware->set_hook_loader($loader);
        $this->assertSame($loader, \pc_stock_man_v1\Gin0115\WPUnit_Helpers\Objects::get_property($middleware, 'loader'));
    }
    ...
    {
        $middleware = new \pc_stock_man_v1\PinkCrab\Registerables\Registration_Middleware\Registerable_Middleware();
        $container = $this->createMock(\pc_stock_man_v1\PinkCrab\Perique\Interfaces\DI_Container::class);
        $middleware->set_di_container($container);
        $this->assertSame($container, \pc_stock_man_v1\Gin0115\WPUnit_Helpers\Objects::get_property($middleware, 'container'));
    }
    ...
    public function test_doesnt_process_none_registerable_objects() : void
    {
        ...
        $this->assertEquals(0, \pc_stock_man_v1\Gin0115\WPUnit_Helpers\Objects::get_property($loader, 'hooks')->count());
    }
    ...
    public function test_registers_registerable() : void
    {
        ...
        $middleware = new \pc_stock_man_v1\PinkCrab\Registerables\Registration_Middleware\Registerable_Middleware();
        $middleware->set_hook_loader($loader);
        $middleware->process($this->createMock(\pc_stock_man_v1\PinkCrab\Registerables\Taxonomy::class));
        // Extract just the hook handles form the loader.
        $hooks = \pc_stock_man_v1\Gin0115\WPUnit_Helpers\Objects::get_property($loader, 'hooks')->export();
    ...
    public function test_unhandled_registerable_silently_skips() : void
    {
        ...
        $middleware = new \pc_stock_man_v1\PinkCrab\Registerables\Registration_Middleware\Registerable_Middleware();
        $middleware->set_hook_loader($loader);
        $registerable = new class implements \pc_stock_man_v1\PinkCrab\Registerables\Registration_Middleware\Registerable
        {
        };
