...
class Test_Post_Page_Meta_Box extends \WP_UnitTestCase
{
    ...
    {
        $post_meta_box = \array_values($this->meta_box_inspector->filter(function ($meta_box) {
            return $meta_box->post_type === 'post' && $meta_box->id === 'post_page_mb';
        }))[0];
        $page_meta_box = \array_values($this->meta_box_inspector->filter(function ($meta_box) {
