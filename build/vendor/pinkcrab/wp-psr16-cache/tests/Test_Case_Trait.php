 ...
 */
namespace PC_Woo_Stock_Man\PinkCrab\WP_PSR16_Cache\Tests;
...
trait Test_Case_Trait
{
    ...
    {
        $this->cache->setMultiple(array('a1' => 'A one', 'b2' => 'B two'));
    ...
    {
        $this->cache->setMultiple(array('c3' => 'C three', 'd4' => 'D four'));
        $this->cache->set('e5', 'E five');
        $values = $this->cache->getMultiple(array('c3', 'd4', 'e5', 'f6'));
    ...
    {
        $this->cache->setMultiple(array('g7' => 'G seven', 'h8' => 'H eight'));
        $values = $this->cache->getMultiple(array('g7', 'h8', 'i9'), 'FALLBACK');
    ...
    {
        $this->cache->setMultiple(array('i9' => 'I Nine', 'j10' => 'J Ten'));
    ...
    public function test_valid_key() : void
    {
        ...
        $valid = array('3e3dc99bedcd3c30416fed1e803cfc8c', 'bcc17601257888da30fbad56eb25dc60', \md5(\strval(\time())), \md5('124564'), \md5(\strval(12 + 6767)));
        foreach ($valid as $key) {
            $this->assertTrue($cache->valid_key($key));
        }
        $invalid = array('@at', '{open', '}close', '/me\\');
