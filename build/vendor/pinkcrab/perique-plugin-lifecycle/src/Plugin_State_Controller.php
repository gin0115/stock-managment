...
class Plugin_State_Controller
{
    ...
    {
        return \array_filter($this->state_events, function ($e) use($state) : bool {
    ...
    protected function get_called_file() : string
    {
        ...
        for ($i = 0; $i < $backtrace_count; $i++) {
            if ($backtrace[$i]['function'] === __FUNCTION__ && \array_key_exists('class', $backtrace[$i]) && $backtrace[$i]['class'] === \get_class() && \array_key_exists($i + 1, $backtrace) && \array_key_exists('file', $backtrace[$i + 1])) {
