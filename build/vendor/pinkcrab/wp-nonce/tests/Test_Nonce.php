 ...
 */
namespace PC_Woo_Stock_Man\PinkCrab\Nonce\Tests;

use PC_Woo_Stock_Man\PinkCrab\Nonce\Nonce;
use PC_Woo_Stock_Man\PinkCrab\PHPUnit_Helpers\Output;
use PC_Woo_Stock_Man\PinkCrab\PHPUnit_Helpers\Reflection;
class Test_Nonce extends \WP_UnitTestCase
{
    public function test_can_create_nonce() : void
    {
        $nonce = new \PC_Woo_Stock_Man\PinkCrab\Nonce\Nonce('test');
        $this->assertInstanceOf(\PC_Woo_Stock_Man\PinkCrab\Nonce\Nonce::class, $nonce);
        // Ensure the properties are set.
        $this->assertEquals('test', \PC_Woo_Stock_Man\PinkCrab\PHPUnit_Helpers\Reflection::get_private_property($nonce, 'action'));
        $this->assertNotEmpty(\PC_Woo_Stock_Man\PinkCrab\PHPUnit_Helpers\Reflection::get_private_property($nonce, 'nonce_token'));
    }
    /**
    ...
    public function test_nonce_added_to_url() : void
    {
        ...
        $nonce = new \PC_Woo_Stock_Man\PinkCrab\Nonce\Nonce('test_nonce_added_to_url');
        // Using custom param.
        $this->assertEquals("https://www.test.com?custom_nonce={$nonce->token()}", $nonce->as_url($simple_url, 'custom_nonce'));
        $this->assertEquals("https://www.test.com?spoon=man&other=thing&custom_nonce={$nonce->token()}", $nonce->as_url($complex_url, 'custom_nonce'));
        // WIthout custom parameter
        $this->assertEquals("https://www.test.com?_wpnonce={$nonce->token()}", $nonce->as_url($simple_url));
        $this->assertEquals("https://www.test.com?spoon=man&other=thing&_wpnonce={$nonce->token()}", $nonce->as_url($complex_url));
    }
    /**
    ...
    {
        $nonce = new \PC_Woo_Stock_Man\PinkCrab\Nonce\Nonce('serialised');
