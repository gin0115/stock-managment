...
trait BladeOneHtml
{
    ...
    public function useBootstrap3($useCDN = \false) : void
    {
        ...
        $bs3 = ['button' => 'btn', 'input' => 'form-control', 'textarea' => 'form-control', 'select' => 'form-control', 'file' => 'form-control-file', 'range' => 'form-control-range', 'ul' => 'list-group', 'ul_item' => 'list-group-item', 'ol' => 'list-group', 'ol_item' => 'list-group-item', 'table' => 'table', 'alert' => 'alert', 'container' => 'container-fluid', 'row' => 'row', 'col' => 'col'];
        $this->defaultClass = \array_merge($this->defaultClass, $bs3);
        if ($useCDN) {
            $this->addCss('<link rel="stylesheet" ' . 'href="https://stackpath.bootstrapcdn.com/bootstrap/3.4.1/css/bootstrap.min.css" ' . 'integrity="sha384-HSMxcRTRxnN+Bdg0JdbxYKrThecOKuH5zCYotlSAcp1+c8xmyTe9GYg1l9a69psu" ' . 'crossorigin="anonymous">', 'bootstrap');
...
<div class="form-check">
    ...
    protected function compilePagination($expression)
    {
        if ($this->style !== 'bootstrap4' && $this->style !== 'bootstrap3' && $this->style !== 'bootstrap5') {
            ...
            use useBootstrap3(), useBootstrap4() or useBootstrap5()', \true);
            return '';
        }
        $args = $this->getArgs($expression);
    ...
    protected function compileItems($expression) : string
    {
        ...
        $nameKey = '$_msgk';
        $html = $this->phpTag . ' ' . $nameOG . '=\'\';  foreach(' . $name . ' as ' . $nameKey . '=>' . $args['alias'] . ') {' . "\n";
        ...
        $htmlItem = $this->render($args, $parent['type'] . '_item', $result);
        $htmlItem = \str_replace('{{checked}}', $this->phpTag . ' echo (' . @$args['value'] . "=={$parent['value']})?'{$checkedname}':''; ?>", $htmlItem);
        $html .= $htmlItem;
    ...
    protected function utilGenEndContainer($nameTag)
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@' . $nameTag, "Missing @{$nameTag} or so many @{$nameTag}", \true);
        }
        return $this->pattern[$parent['type'] . '_end'];
    }
    /** @noinspection PhpUnused */
    ...
    protected function compileEndMessages()
    {
        $parent = @\array_pop($this->htmlItem);
        if ($parent === null) {
            $this->showError('@endmessages', 'Missing @messages or so many @endmessages', \true);
        }
        return $this->pattern[$parent['type'] . '_end'];
    }
    /** @noinspection PhpUnused */
